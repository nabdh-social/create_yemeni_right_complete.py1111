  ...
  Future<void> _handleLogin() async {
    ...
    if (success && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(ref.read(authProvider).errorMessage ?? 'Error'), backgroundColor: AppTheme.yemeniRed),
      );
    }
  }
              ...
              Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 24),
                    ElevatedButton(onPressed: authState.isLoading ? null : _handleLogin, child: authState.isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('Login')),
                    const SizedBox(height: 24),
...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  Future<void> _handleRegister() async {
    ...
    if (success && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(ref.read(authProvider).errorMessage ?? 'Error'), backgroundColor: AppTheme.yemeniRed));
    }
  }
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 24),
                ElevatedButton(onPressed: authState.isLoading ? null : _handleRegister, child: authState.isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('Create Account')),
                const SizedBox(height: 16),
...
class _OtpScreenState extends ConsumerState<OtpScreen> {
  ...
  Future<void> _verifyOtp() async {
    ...
    if (success && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(ref.read(authProvider).errorMessage ?? 'Invalid code'), backgroundColor: AppTheme.yemeniRed));
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_isCodeSent) ...[
                ...
                const SizedBox(height: 24),
                ElevatedButton(onPressed: authState.isLoading ? null : _sendOtp, child: authState.isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text('Send Code')),
              ] else ...[
                ...
                const SizedBox(height: 32),
                ElevatedButton(onPressed: authState.isLoading ? null : _verifyOtp, child: authState.isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text('Verify')),
                const SizedBox(height: 16),
...
class ProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Profile'), actions: [IconButton(icon: const Icon(Icons.settings), onPressed: () {})]), body: SingleChildScrollView(child: Column(children: [
      ...
      const SizedBox(height: 16),
      Text(userData?['fullName'] ?? 'User', style: const TextStyle(fontFamily: 'Cairo', fontSize: 24, fontWeight: FontWeight.bold)),
      Text(userData?['userType'] == 'lawyer' ? 'Lawyer' : 'Citizen', style: TextStyle(color: Colors.grey.shade600)),
      const SizedBox(height: 24),
      ...
      ListTile(leading: const Icon(Icons.logout, color: AppTheme.yemeniRed), title: const Text('Logout', style: TextStyle(color: AppTheme.yemeniRed)), onTap: () => _showLogoutDialog(context, ref)),
      const SizedBox(height: 24),
    ])));
  }
  ...
  void _showLogoutDialog(BuildContext context, WidgetRef ref) {
    showDialog(context: context, builder: (context) => AlertDialog(title: const Text('Logout'), content: const Text('Are you sure?'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')), ElevatedButton(onPressed: () { ref.read(authProvider).signOut(); Navigator.pop(context); context.go('/login'); }, style: ElevatedButton.styleFrom(backgroundColor: AppTheme.yemeniRed), child: const Text('Logout'))]));
  }
}
""")
